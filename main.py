...
BOT_TOKEN = os.environ.get("BOT_TOKEN","8860968654:AAFvSnewn17ygVwdAviMOnX6I739yPo83PI")
...
def today():
    ...
    def mkb():return ReplyKeyboardMarkup([["Yangi yozuv"]],resize_keyboard=True)
        def pkb():
            ...
            async def start(u,c):await u.message.reply_text("Salom Aziza!",reply_markup=mkb())
...
async def cancel(u,c):await u.message.reply_text("Bekor.",reply_markup=mkb());return ConversationHandler.END
    ...
    if WEBHOOK_URL:
                app.run_webhook(listen="0.0.0.0",port=PORT,webhook_url=f"{WEBHOOK_URL}/{BOT_TOKEN}",url_path=f"/{BOT_TOKEN}",drop_pending_updates=True)
else:
